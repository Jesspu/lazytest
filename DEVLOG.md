...
### 2026-07-29
- **Monorepo Workspace Awareness**: Added `runner/workspace.go` with `DiscoverWorkspaces`, which parses npm/yarn `workspaces` arrays from `package.json` (plain and Yarn object forms) and `packages` lists from `pnpm-workspace.yaml` via a zero-dependency line parser. Glob patterns are expanded with `filepath.Glob` and each matched directory is resolved to a `Workspace` with its own auto-detected `Config`. Added `LoadConfigForPath` to `runner/config.go` for per-path config resolution using the workspace list. Updated `PrepareJob` to accept `[]Workspace` and route each test file to its package's runner. Stored `Workspaces` on the `Engine` struct, populated at startup and rebuilt on any config file change. Welcome banner now shows workspace count for monorepos. All changes covered by new `TestWorkspaceRouting` and `TestDiscoverWorkspaces` tests.
- **Welcome Banner**: Populated `engine.State.WelcomeMessage` at startup via `generateWelcome()`, displaying the auto-detected runner name, command template, and concurrency level in the output pane before any test runs. Added `HasAnyOutput()` accessor and updated `syncViewportOutput` to show the styled banner whenever no test has produced output yet. Banner auto-dismisses on first test run. Added `welcomeStyle` (dimmed `"241"` foreground) to `ui/styles.go`.
